...
export default function ResumoHoje() {
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            const getResumoHoje = async () => {
                const Data = new Date()
                const mes = Data.getMonth()
                const dia = Data.getDate()
                const dataHoje = Data.getFullYear() + "-" + (mes.toString().length === 1 ? "0" + `${mes + 1}` : mes + 1) + "-" + (dia.toString().length === 1 ? "0" + `${dia}` : dia)
                const result = await fetch(`https://baldosplasticosapi.herokuapp.com/notas/${dataHoje}/${dataHoje}/${await AsyncStorage.getItem("token")}`)
